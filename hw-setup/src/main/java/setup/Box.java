...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        // Your code goes here.  Remove the exception after you're done.
        // throw new RuntimeException("Method not implemented");
        if (b.getVolume() + this.ballContainer.getVolume() > this.maxVolumn) {
            return false;
        }
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        ...
        List<Ball> itrList = new ArrayList<>();

        while (itr.hasNext()) {
            itrList.add(itr.next());
        }

        itrList.sort(Comparator.comparingDouble(Ball::getVolume));
