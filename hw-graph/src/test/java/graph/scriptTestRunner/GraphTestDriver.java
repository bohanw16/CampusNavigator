...
public class GraphTestDriver {
    ...
    private void listChildren(String graphName, String parentName) {
        ...
        List<Graph<String, String>.Edge> outgoingEdges = graph.getChildren(parentName);
        for (Graph<String, String>.Edge edge : outgoingEdges) {
            String child = edge.getDestination();
            if (!childrenMap.containsKey(child)) {
                childrenMap.put(child, new HashSet<>());
            }
            childrenMap.get(child).add(edge.getLabel());
        }
        TreeSet<String> childrenNames = new TreeSet<>(childrenMap.keySet());
        output.print("the children of " + parentName + " in " + graphName + " are:");
        for (String childName : childrenNames) {
            TreeSet<String> edges = new TreeSet<>(childrenMap.get(childName));
            for (String edge : edges) {
                output.print(" " + childName + "(" + edge + ")");
            }
        }
        output.println("");
    }
